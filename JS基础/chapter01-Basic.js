...
// const 的注意点：
// 1. const 声明的值不能更改，并且在使用const声明变量时必须进行初始化
// 2. 但是对于[引用数据类型]，const声明的变量里面存放的不是值！而是地址！
// 3. 引用数据类型：对象（比如：数组Array）

// let myArray = ['All', 'In', 'Sync'];
const myArray = ['All', 'In', 'Sync'];
myArray.push('Twilight-eleven');       // 只是在数组末尾追加了一个字符串，而数组地址不变，那就还是他自己本身
console.log(myArray);
